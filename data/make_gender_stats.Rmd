...
Here we are processing a dataset from the World Bank on gender and
inequality.

I got the dataset from these pages.

* [https://data.worldbank.org/data-catalog/gender-statistics](https://data.worldbank.org/data-catalog/gender-statistics)

The dataset that I'm using is one I downloaded sometime soon before 16 September 2017.  I'm using that version of the dataset because it has entries for health expenditure per capita, that current downloads do not have.

If you want tolatest version of these data, you can get them as a zip file [from that
site](http://databank.worldbank.org/data/download/Gender_Stats_csv.zip). 

Here is the September 2017 version of the data, extracted from the corresponding zip file:

* [2017 Gender_StatsData.csv](https://ndownloader.figshare.com/files/17803301)

The last is the link I'm using below.
...
Next we look at the first few rows of the data frame:
...
```{python}
gender_stats_2014 = valid_gender_stats.copy()
gender_stats_2014.loc[:, 'gdp'] = gdp_us_billion
gender_stats_2014 = gender_stats_2014.rename(columns={'gdp': 'gdp_us_billion'})
# We will later need to change the name for later output in the data dictionary.
renamers = [t if t != ('GDP (current US$)', 'gdp')
            else ('GDP (current US billion $)', 'gdp_us_billion')
            for t in renamers]
gender_stats_2014.head()
```

Rename country columns to harmonize with format of other names:

```{python}
gender_stats_2014 = gender_stats_2014.rename(
...
```{python}
gender_stats_2014.to_csv('gender_stats_2012_16.csv', index=False)
```

Read it back in to check it's what we were expecting:

```{python}
gs_back = pd.read_csv('gender_stats_2012_16.csv')
gs_back.head()
```
