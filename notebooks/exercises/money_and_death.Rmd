...
income bracket, and those two did give an answer to the death penalty
question.  Do do this, we use Pandas `dropna` method of the data
frame, to drop all rows that have missing values.
...
Do estimate how variable the mean differences can be, for such random sampling, we simulate this sampling by pooling, then permuting, the income values that we have, from the two groups.
...
To do the random sampling we permute the values, so the `pooled` vector is now a random mixture of the two groups.
