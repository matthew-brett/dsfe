...
Let us refine our cell to print `Negative` if `x` is a negative number.
We can do this by adding an `elif` clause, where `elif` if Python's
...
What if `x` is 0? To deal with this case, we can add another `elif`
clause:
