...
## The function is a value too

You may have noticed the section above, where talked about functions having names.  In fact, function names are just variables.  Consider the `range` function.  Here I use `range` on its own.  In fact `range` is just a variable name, where the variable points to the internal representation of the function (procedure).   I can use `range` on its own, just as I could use any other variable name, and it is an expression, that returns the value of `range`:

```{python}
range
```

Likewise, `biased_coin` above is a variable name, where the variable points to the internal representation of the `biased_coin` procedure:

```{python}
biased_coin
```

What do you think you would get from the following?

```{python tags=c("raises-exception")}
biased_coin + 2
```

Why is this?  Python evaluates the first expression `biased_coin`; it
is a variable pointing to a internal representation of a function, so
the first value is a function.  The second value is IR of int 2.  It
tries to add the IR of a function to the IR of an int, and fails.

This reminds us that, in order to apply the procedure, we need to *call* the function.  We do this, as you know, by using parentheses after the function, to make a *call expression*:
...
As we know, we can read this as "The variable 'a' gets the value IR of
2".
...
```{python}
my_function = biased_coin
```

Read this in the normal way: "The variable 'my_function' gets the value
...".  What is the value?  The value is the result of evaluating
`biased_coin`, and you know what that is, from above.  It's the
internal representation (IR) of the function.

So we read the statement as "The variable 'my_function' gets the value IR of the function originally called 'biased_coin'".

What do you think we would see if we evaluate this expression:
`my_function()`?

What will Python do?  It first evaluates `my_function`, and gets the value, which is the IR of the function.  Then it sees the parentheses following, so it *calls* the function.  What function?  The function originally called "biased_coin".

```{python}
my_function()
```

Remember that the expression `my_function` is not the same as the expression `my_function()`.  `my_function` evaluates to the IR of the function.  `my_function()` evaluates to result of *calling* the IR of the function.
