...
ingredients: two eggs; butter; and cheese \- to the meal
- a cheese omelette.
...
I say `range` has a procedure, but we can't see what the procedure is,
it's buried inside internal workings.

Now we are going to write our own functions, were we can see the procedure.
...
As for [for loops](../05/iteration), the colon signifies that the next
...
We can see this if we run the following code, from our usual top-level
...
Remember, above that Python knows that the arguments to a function are expressions, and evaluates them, before running the function.
...
If we give do not give it exactly two arguments, we get an error.
